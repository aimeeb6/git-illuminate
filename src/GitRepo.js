...
ipcMain.on('open-repo', (event, arg)=> {
  ...
  let reponame = arg.substring(arg.lastIndexOf("/") + 1);
  openRepos[reponame] = new GitRepo(arg);
  openRepos[reponame].setRepo().then(() => {openRepos[reponame].refeshRepo().then((commits) => {event.returnValue = commits})})
